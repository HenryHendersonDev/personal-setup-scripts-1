...
alias startXray='if [ -f /usr/local/xray/config.json ]; then nohup /usr/local/xray/xray -config /usr/local/xray/config.json > /usr/local/xray/xray.log 2>&1 & echo $! > /usr/local/xray/xray.pid && /usr/local/xray/script/enable-xray-proxy.sh; else echo "Xray config not found"; fi'

alias stopXray='if [ -f /usr/local/xray/xray.pid ] && kill -0 $(cat /usr/local/xray/xray.pid) 2>/dev/null; then kill $(cat /usr/local/xray/xray.pid) && rm -f /usr/local/xray/xray.pid && /usr/local/xray/script/disable-xray-proxy.sh; else echo "Xray not running"; fi'
